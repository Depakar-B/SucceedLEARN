...
?>
<section class="sl-section sl-section--alt sl-testimonials">
	<div class="sl-wrap">
		<p class="sl-eyebrow"><?php esc_html_e( 'What clients say', 'succeedlearn-amp' ); ?></p>
		<h2 class="sl-h2">
			<?php
			echo wp_kses(
				__( 'From teams who <span>rolled it out</span>', 'succeedlearn-amp' ),
				array( 'span' => array() )
			);
			?>
		</h2>
			...
			<?php foreach ( $testimonials as $item ) : ?>
				<figure class="sl-testimonials__card">
					...
					<?php if ( ! empty( $item['note'] ) ) : ?>
						<p class="sl-testimonials__note sl-gwct-testimonials-note"><?php echo esc_html( $item['note'] ); ?></p>
					<?php endif; ?>
					<figcaption>
