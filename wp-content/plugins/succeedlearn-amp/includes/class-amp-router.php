...
class Amp_Router {
	...
	public function init() {
		...
		add_action( 'wp', array( $this, 'maybe_bounce_skipped_amp' ), 0 );
		add_action( 'wp', array( $this, 'maybe_redirect' ), 1 );
		add_action( 'wp', array( $this, 'maybe_redirect' ), 99 );
		add_action( 'template_redirect', array( $this, 'maybe_bounce_skipped_amp' ), 0 );
	 ...
	 * Pages with skip_amp get hide-amp so AMPforWP leaves the normal theme alone.
	 */
	public function enable_amp_post_meta_for_plugin_pages() {
		...
		$front_id = absint( get_option( 'page_on_front' ) );
		if ( $front_id && ! Config::page_type_skips_amp( 'home' ) ) {
			$enable_ids[] = $front_id;
		}
		...
		foreach ( Config::get_amp_page_map() as $type => $map ) {
			...
			$bucket = Config::page_type_skips_amp( $type ) ? 'hide' : 'enable';
			$ids    = array();
			...
			if ( 'hide' === $bucket ) {
				$hide_ids = array_merge( $hide_ids, $ids );
			} else {
				$enable_ids = array_merge( $enable_ids, $ids );
			}
		}

		foreach ( array_unique( array_filter( $enable_ids ) ) as $post_id ) {
			update_post_meta( $post_id, 'ampforwp-amp-on-off', 'enabled' );
		}
		foreach ( array_unique( array_filter( $hide_ids ) ) as $post_id ) {
			update_post_meta( $post_id, 'ampforwp-amp-on-off', 'hide-amp' );
		}
	}
	...
	/**
	 * Bounce /amp/ and ?amp=1 to the normal theme URL for skipped or unmapped pages.
	 */
	public function maybe_bounce_skipped_amp() {
		...
		$theme_url = $this->get_theme_permalink();
		if ( ! $theme_url ) {
			return;
		}
		...
		if ( untrailingslashit( $current ) === untrailingslashit( $theme_url ) ) {
			return;
		}

		nocache_headers();
		header( 'Cache-Control: private, no-store, no-cache, must-revalidate, max-age=0' );
		wp_safe_redirect( $theme_url, 302 );
		exit;
	}
	...
	/**
	 * Canonical non-AMP (theme) URL for the current request.
	 *
	 * @return string
	 */
	public function get_theme_permalink() {
		$page_type = $this->resolve_request_page_type();
		...
		$permalink = remove_query_arg( 'amp', $permalink );
		return $permalink;
	}
	...
	public function get_amp_permalink() {
		$permalink = $this->get_theme_permalink();
		if ( ! $permalink ) {
			return '';
		}
