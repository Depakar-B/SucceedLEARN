...
class Performance_Optimizer {
	...
	public function get_optimized_css( $page_type, $style_files = array() ) {
		...
		$base        = SUCCEEDLEARN_AMP_TEMPLATES_DIR . 'styles/';

		$newest_source = 0;
		foreach ( $style_files as $file ) {
			$name     = sanitize_file_name( (string) $file );
			$css_path = $base . $name . '.css';
			$php_path = $base . $name . '.php';
			if ( is_readable( $css_path ) ) {
				$newest_source = max( $newest_source, (int) filemtime( $css_path ) );
			} elseif ( is_readable( $php_path ) ) {
				$newest_source = max( $newest_source, (int) filemtime( $php_path ) );
			}
		}
		...
		if (
			...
			&& (int) filemtime( $cache_file ) >= $newest_source
		) {
		...
		foreach ( $style_files as $file ) {
			...
			if ( is_readable( $css_path ) ) {
				// Prefer plain CSS files — same as succeedlearn_amp_include_style_partial().
				$css .= (string) file_get_contents( $css_path ); // phpcs:ignore WordPress.WP.AlternativeFunctions.file_get_contents_file_get_contents
				continue;
			}
